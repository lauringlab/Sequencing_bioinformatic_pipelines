...
def main():
    ...
    consensus_files = args.run + "/data/ivar_output/consensus/*.fa"
    all_fasta = list()
    all_fasta_full = list ()
    tmp_filename = args.run + "/data/ivar_output/" +  "tmp.consensus.fasta"
    tmp_filename2 = args.run + "/data/ivar_output/" +  "tmp.consensus2.fasta"
    final_filename = args.run +"/" + args.run + ".90.consensus.fasta"
    final_filename_all = args.run + "/" + args.run + ".full.consensus.fasta"

    for file in glob.glob(consensus_files):

        print("Working on file: " + file)
        filename_only = file.split("/")[4]
...
### Combine coverage files ###
    cov_files = args.run +"/data/ivar_output/coverage/*.csv"
    cov_filename = args.run+"/" + args.run + ".coverage.csv"
