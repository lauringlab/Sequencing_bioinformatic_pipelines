...
def main():
	...
	for id in IDS:
		collect_cmd = "cat " + args.prefix + "/"+ id + "/amended_consensus/*.fa > " + id + "/" + id + ".consensus.fasta"
	...
	for id in IDS:        
		for filename in glob.glob (args.prefix + "/" + id + "/B_*.fasta"):
	...
	collect_cmd_B = "cat " + args.prefix + "/*/*_B.fasta > "  + args.prefix + "/" + args.prefix + ".B.consensus.fa"

	if args.mode == "run":

		subprocess.call(collect_cmd_B, shell = True)

	if args.mode == "run":
		for filename in  glob.glob( args.prefix + "/"+ args.prefix +".B.consensus.fa"):
			all_fasta = list ()
			tmp_filename = filename + ".tmp"
